...
<div class="easyui-layout" fit="true" border="false">
    <div region="center" border="false"> 
		 ...
		 <table id="ttPart" class="easyui-datagrid" fit="true" border="true"
				...
				url="${pageContext.request.contextPath}/vehicle/queryAllParts"
			    iconCls="icon-save" 
			    pageSize="20"
				...
				<table class="mytb"> 
					<tr>
						...
						<td> 
    						...
    						data-options="valueField:'id',textField:'partcategory',url:'<%=request.getContextPath() %>/baseData/searchLJCategory'" validateOnCreate="false" required="true"  style="width:200px;height:30px;"/>							　
						</td>
					</tr>
					<tr>
						<td>零件名称</td>
					...
					<tr>
						...
						<td> 
    						...
    						data-options="valueField:'id',textField:'suppliername',url:'<%=request.getContextPath() %>/baseData/searchSuppliers'" validateOnCreate="false" required="true"  style="width:200px;height:30px;"/>							　
						</td>
					</tr>
					<tr>
						<td>仓库</td>
						<td> 
						   <input id="storage.warehouseid" class="easyui-combobox" name="storage.warehouseid" prompt="请选择仓库"
    						data-options="valueField:'id',textField:'warehousename',url:'<%=request.getContextPath() %>/baseData/searchWareHouse'" validateOnCreate="false" required="true"  style="width:200px;height:30px;"/>							　
						</td>
					</tr>
					<tr>
						<td>详细位置</td>
    ...
    <script type="text/javascript">      	
     	...
     	function saveMainitem(){
     		$.messager.progress();  
	  		$('#fmmainitem').form('submit', {
	  			url:"<%=request.getContextPath()%>/baseData/storagePart",
	  			onSubmit: function(){
     	...
     	function del(partid){     		 
     		 $.messager.confirm("操作提示","是否确认删除?",function(r){
        		 if(r){ 
                    $.ajax({
                    	url:"<%=request.getContextPath()%>/baseData/delPart",
                    	type:"post",
                    	data:{
                    		partid:partid
                    	},
