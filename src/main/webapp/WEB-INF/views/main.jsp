    ...
    <div id="aa" class="easyui-accordion" fit="true" border="false" class="menuItem">
       ...
       <c:forEach items="${allprivilege.elements}" var="item">
         <div title="&nbsp;&nbsp;&nbsp;&nbsp;${item.title}">
            <ul class="easyui-tree tr" data-options="data:${item.dataJSON}">
            </ul>
        </div>
       </c:forEach>
    </div>
...
<div region="center"  border="false">
    <div class="easyui-tabs" fit="true" id="tt">
        <div title="首页" data-options="iconCls:'icon-application_home'">
			<iframe  style="width:100%;height:100%;border:none;" src="<%=request.getContextPath()%>/index/dis?url=/fm/reportStatistics.jsp"></iframe>
        </div>
    </div>
</div>
<!-- 版权部分 -->
	...
	**/
	function exit(){
		$.messager.confirm('确认', '你好,${loginbean.username}!你确定要退出系统登录吗?', function(r){
			if (r){
				window.location.href="${pageContext.request.contextPath}/users/${loginbean.accountnumber}/logout"
			}
		});
	}
    ...
    $(function(){
        ...
        function addTab(iconCls,title, url){
            if ($('#tt').tabs('exists', title)){
                ...
                var content = '<iframe scrolling="auto"  frameborder="0"  src="${pageContext.request.contextPath}/index/dis?url='+url+'" style="width:100%;height:100%;margin-bottom:-3px;"></iframe>';
                $('#tt').tabs('add',{
                    title:title,
                    content:content,
                    closable:true,
