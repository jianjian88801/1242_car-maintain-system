...
<div class="easyui-layout" fit="true" border="false">
    <div region="center" border="false">
        <table fit="true" id="dg" class="easyui-datagrid" title="用户 信息" 
               ...
               data-options="url:'${pageContext.request.contextPath}/base/queryUser'"
        ...
        <div id="tb" style="padding:5px">
            ...
            &nbsp;&nbsp;
            <a href="#" id="refreshall" iconCls="icon-arrow_refresh" onclick="refresh()" class="easyui-linkbutton">刷新所有</a>
             &nbsp;&nbsp;
...
<div id="usercomment">
	...
	<div id="commentctrl">
		<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-cancel'" onclick="$('#usercomment').dialog('close')">关闭</a>
	</div>
</div>
<div id="userdialog">
    			 ...
    			 <tr>
    			  ...
    			  <tr>
    			 	...
    			 	<td>
    					...
    					data-options="valueField:'id',textField:'deptname',url:'<%=request.getContextPath() %>/base/queryAllSector'" required="required">
    			 </tr>
    			  <tr> 
...
<script type="text/javascript">
	...
	function opfm(value,row,index){
		...
		let d = "<button onclick=showcomment("+index+") style='border:none;cursor:pointer;border-radius:3px;background:green;color:white;'>查看评价</button>";
	...
	function loadAllPrivileges(){
		$.ajax({
			url:"<%=request.getContextPath()%>/base/queryAllPrivilege",
			type:"post",
			dataType:"json"
		}).done(function(data){   
	...
	function suresubmit(){
		var url = ""; 
		if(flag=="0"){// 添加
			url = "<%=request.getContextPath()%>/base/c/modiUser";
		}else if(flag=="1"){// 修改
			url = "<%=request.getContextPath()%>/base/u/modiUser";
		}	
		$.messager.progress();	 
	...
	function changeStatus(userinfoid,userflag){
		$.messager.confirm('操作提示', '你确定要继续该操作吗?', function(r){
			if (r){
				$.ajax({
					url:"<%=request.getContextPath()%>/base/d/modiUser",
					type:"post",
					data:{
	...
	function queryAllUserPrivilege(accountnumber){
		var result = "";
		$.ajax({
			url:"<%=request.getContextPath()%>/base/queryAllUserPrivilege",
			type:"post",
			data:{
	...
	function showcomment(index) {
		...
		$.ajax({
			url:"<%=request.getContextPath()%>/baseData/queryallComment",
			type:"post",
			dataType:"json"
		}).done(function(data){
			console.log(data);
			// 在fieldset中添加权限
			var row = getRow(index);
			$("#usercomment_header").html("");
			var cont = "";
			for(var i = 0;i<data.length;i++){
				console.log(row.userinfoid);
				console.log(data.userid);
				console.log(" ");

				if(row.userinfoid == data[i].userid){
					str="<tr> <th style='width: 30%'>"+data[i].username +" </th> <th style='width: 30%'>"+data[i].comment +" </th> </tr>";
					cont += str;
				}
			}
			$("#usercomment_header").html(cont);
			$("#usercomment").dialog('open');
		});
	}
